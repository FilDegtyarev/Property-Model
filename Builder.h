...
class PMBuilder<Data<DataArgs...>, Value<ValueArgs...>, Output<OutputArgs...>> {
  ...
  public:
	...
	void add_constraint(std::vector<std::string> variables, int priority, bool enable) {
		...
		if (enable) {
			current_constraint_ = std::make_unique<Constraint>(refs, Priority{priority}, Status::enabled);
		} else {
			current_constraint_ = std::make_unique<Constraint>(refs, Priority{priority}, StatusType{0});
		}
		return;
	}
	...
	void add_method(std::function<std::tuple<OutputTypes...>(Types...)> function, std::vector<std::string> inputs,
		...
		std::function<void()> method = property_model_->bind_method(function, inputs, outputs);
